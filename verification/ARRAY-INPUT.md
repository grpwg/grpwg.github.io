...
- 2026-09-25：松手速度上限 `MAX_SLIDE_SPEED = 18` 格/秒（`ArchiveDrag.releaseVelocity`，两轨道等比缩放以保留滑行方向）。快慢甩动的距离差保留（上限仍高于快慢比断言所需的 16 格/秒，再低会抹掉快慢惯性差），但极端快甩不再高速滑行，避免低配设备每帧跨越多格、集中触发选档（含标签画布重绘）而卡顿；主动拖动仍与指针 1:1 跟随。
