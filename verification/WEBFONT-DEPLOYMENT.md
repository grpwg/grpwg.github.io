...
- （历史记录）后续仅本项目 Vercel 构建从当前正式域名恢复同一份字体及原 CSS，逐文件验证 SHA-256；2026-09-25 起改为仅在设置 `RHINE_WEBFONT_SOURCE` 时恢复。恢复失败中止构建，保留原部署；其他项目没有本地字体时沿用固定文字图形。
- （历史记录，2026-09-25 起本分支不再依赖该域名）正式域名 `https://rhine.lubeiluchen.cc/` 的 PWA 版本为 `0aa8a1c41186b335`。从空目录执行恢复脚本，成功下载并验证全部四个资源，线上 PWA 清单包含三份字体。
